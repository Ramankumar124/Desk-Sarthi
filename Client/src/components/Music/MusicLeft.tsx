...
import { IoMusicalNote, IoPauseOutline, IoPlayOutline, IoSearch } from "react-icons/io5";
...
const MusicLeft = ({
 ...
 const handlePlay=async (track:Track)=>(
  playTrack(track?.uri,"Searched Track")
  .then(() => {
    setsearchInput("");
    settoggleSearch(false);
  })
  ...
  return (
    <div id="music-left" className="w-full h-full md:w-1/2 bg-primary">
      <div className="bg-neutral-800 rounded-xl p-6 transition-all">
        <button className="bg-red-400 p-2 rounded mb-4" onClick={spotifyLogin}>
          "Login to Spotify"
        </button>
        ...
        <div className={`relative ${toggleSearch?"mb-6":"mb-0"}`}>
          ...
          <Button onClick={searchSong} className="px-2 py-0 absolute right-3 top-1.5 text-xs">
...
{!toggleSearch ? <div>
        ...
        </div>:        <div className="space-y-4 h max-h-[300px] overflow-y-scroll">
        {
        searchedTracks && searchedTracks.map((track:Track)=>(
          <div
